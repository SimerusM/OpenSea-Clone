...
const Collection = () => {
  ...
  // Receives info on the nft module stored in the sanity database
  ...
  useEffect(() => {
    ...
    // IIFE, this is equivalent to a lambda function
    ;(async () => {
  ...
  useEffect(() => {
    // If marketplacemodule doesn't exist, return none
  ...
  // Pulls data from sanity database
  const fetchCollectionData = async (sanityClient = client) => {
    ...
    // Fetching query request on marketplace 
    const collectionData = await sanityClient.fetch(query)

    console.log(collectionData, '🔥')

    // the query returns 1 object inside of an array
    // Storing all returned data from query in here
